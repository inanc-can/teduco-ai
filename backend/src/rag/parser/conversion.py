...
class DoclingPDFParser:
    ...
    def __init__(self) -> None:
        """
        Initialize the DoclingPDFParser with default pipeline options.
        """
        logging.info("Initializing DoclingPDFParser...")
        self.pipeline_options = PdfPipelineOptions(
            do_ocr=True,
            ocr_options=EasyOcrOptions(force_full_page_ocr=True, lang=["de", "en"]),
    ...
    def convert_document(self, source: Union[Path, bytes, str, DocumentStream], name: Optional[str] = None) -> ConversionResult:
        """
        Convert a PDF source into a Docling ConversionResult.

        Args:
            source: Either a Path/str to a file, raw PDF bytes (e.g. requests.Response.content),
                    or a DocumentStream.
            name: Optional filename to use when source is bytes or a stream (used by conversion.input.file).
        ...
        Example:
            ...
            conversion = parser.convert_document(pdf_bytes, name="aptitude-assessment-de.pdf")
        """
        ...
        try:
            ...
            if isinstance(source, (bytes, bytearray)):
                ...
                conversion = self.doc_converter.convert(cast(Union[Path, str, DocumentStream], doc_stream))
            # if already a DocumentStream, pass through (cast for the type checker)
            elif isinstance(source, DocumentStream):
                conversion = self.doc_converter.convert(cast(Union[Path, str, DocumentStream], source))
    ...
    def conversion_to_markdown(self, conversion: ConversionResult, out_file_path: Optional[Path] = None) -> str:
        """
        ...
        Args:
            ...
            out_file_path: Optional directory Path where the Markdown file will be written.
                           If provided, the directory will be created if necessary.
        ...
        if out_file_path:
            out_file = out_file_path / "aptitude-assessment.md"
            try:
